...
(() => {
  ...
  // Structure from the user's pasted Direct markup (v2.4.2). Direct rows carry no message IDs:
  ...
  let directEls = new Set(), directReversed = false;
  function hash(text) {
    let h = 0x811c9dc5;
  ...
  function rowSummary(added, total) {
    const kinds = added.slice(0, 8).map(row => row.user ? 'you' : directEls.has(row.el) ? `reply(${modelOf(row.el) || '?'})` : 'reply');
  ...
  globalThis.ArenaAgentDOM = { version: '2.4.2', ROW, DomError, fail, visible, checkBlocks, rows, ended, running,
