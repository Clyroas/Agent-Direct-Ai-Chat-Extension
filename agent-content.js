(() => {
  ...
  function emit(message) { try { owner?.postMessage({ ...message, documentId, adapterVersion: '2.4.2' }); } catch { cleanup(); } }
  ...
  function scan() {
    ...
    try {
      ...
      try { result = D.matchTurn(tx); tx.unclearSince = 0; }
        ...
        if (e?.code === 'AMBIGUOUS_TURN' && D.pageKind() === 'direct') {
          tx.unclearSince ||= Date.now();
          if (Date.now() - tx.unclearSince < DIRECT_SETTLE_MS) { tx.completedText = ''; tx.completeAt = 0; return; }
        }
        throw e;
      }
      checkUrl(tx, result);
