...
export const ADAPTER_VERSION = '2.4.2';
...
export async function attachAgent(tabId, expectedUrl) {
  ...
  if (!chrome.scripting?.executeScript)
    throw new AttachmentError('EXTENSION_UPDATE_REQUIRED', 'The scripting API is unavailable. Reload Arena Auto Chat in chrome://extensions and confirm version 2.4.2 with the scripting permission.');
  ...
  if (checks?.[0]?.result?.version !== ADAPTER_VERSION || checks?.[0]?.result?.domVersion !== ADAPTER_VERSION || checks?.[0]?.result?.attachmentsPolicy !== true)
    throw new AttachmentError('SCRIPT_REGISTRATION_FAILED', 'The bundled Agent script did not register version 2.4.2. Reload the extension and tab. This is an extension attachment problem, not an Arena reply timeout. No prompt was sent.');
